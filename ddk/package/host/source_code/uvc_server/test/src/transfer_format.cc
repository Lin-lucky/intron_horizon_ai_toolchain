...
int TestTransfer::SendNv12Data(void *nv12_data, int len, int nv12_width,
  ...
  if (nv12_data == nullptr || len <= 0 || !IsUvcStreamOn() || !(IsNv12On() ||
    ...
    std::cout << "picture data not transfer " << std::endl;
    return -1;
  }
  if (uvc_server_) {
...
int TestTransfer::SendH264Data(void* h264_data, int len) {
  if (h264_data == nullptr || len <= 0 || !IsUvcStreamOn() || !IsH264On()) {
  ...
  if (uvc_server_) {
    uvccomponent::UvcVideoData video_data;
    memset(&video_data, 0, sizeof(video_data));
    video_data.v_buffer_ = static_cast<char *>(h264_data);
    video_data.v_size_ = len;
    video_data.video_type_ = uvccomponent::CODEC_H264;
    uvc_server_->SendFrame(video_data, false);  // uvc_server should copy data
    return 0;
  }
  return -1;
  ...
  switch (event_type) {
    ...
    case uvccomponent::UVC_STREAM_ON: {
      ...
      if (event_data->video_type_ == uvccomponent::CODEC_NV12) {
        SetNv12IsOn(true);
        SetUvcStreamOn(1);
      } else if (event_data->video_type_ == uvccomponent::CODEC_H264) {
        SetH264IsOn(true);
        SetUvcStreamOn(1);
      } else if (event_data->video_type_ == uvccomponent::CODEC_H265) {
        SetH265IsOn(true);
        SetUvcStreamOn(1);
      } else if (event_data->video_type_ == uvccomponent::CODEC_MJPEG) {
        SetMjpegIsOn(true);
        SetUvcStreamOn(1);
      } else {
        SetNv12IsOn(false);

...
bool ImageCropper::CropWithPadBlack(const ImageToolsFormatData &input,
  ...
  if (!need_padding) {
    ...
    if (need_crop) {
      ...
      // crop和pad过程重复AlignEven
      bool replicate_pad_width = false, replicate_pad_height = false;
      if (IMAGE_TOOLS_RAW_YUV_I420 == input.format_ ||
          IMAGE_TOOLS_RAW_YUV_NV21 == input.format_ ||
          IMAGE_TOOLS_RAW_YUV_NV12 == input.format_) {
        auto is_odd = [](int v) -> bool {
          if (v & static_cast<int>(0X01) != 0) {
            return true;
          } else {
            return false;
          }
        ...
        int crop_width = crop_bottom_right_x - crop_top_left_x + 1;
        int crop_height = crop_bottom_right_y - crop_top_left_y + 1;

        if (is_odd(crop_width) && is_odd(padding_left) &&
            is_odd(padding_right) ||
            is_odd(crop_height) && is_odd(padding_top) &&
            is_odd(padding_bottom)) {
          return false;
        }
        if (!is_odd(crop_width) && is_odd(padding_left) &&
            is_odd(padding_right) ||
            is_odd(crop_width) && !is_odd(padding_left) &&
            is_odd(padding_right) ||
            is_odd(crop_width) && is_odd(padding_left) &&
            !is_odd(padding_right)) {
          replicate_pad_width = true;
        }
        if (!is_odd(crop_height) && is_odd(padding_top) &&
            is_odd(padding_bottom) ||
            is_odd(crop_height) && !is_odd(padding_top) &&
            is_odd(padding_bottom) ||
            is_odd(crop_height) && is_odd(padding_top) &&
            !is_odd(padding_bottom)) {
          replicate_pad_height = true;
        }
      }
      if (!Crop(input,
                crop_top_left_x,
      ...
      if (replicate_pad_width || replicate_pad_height) {
        ImageToolsFormatData pad_tmp_data;
        if (!pad.Pad(tmp_data,
                    padding_left,
                    padding_right,
                    padding_top,
        ...
        if (replicate_pad_width) {
          top_left_x = 1;
          bottom_right_x = pad_tmp_data.width_ - 2;
        }
        if (replicate_pad_height) {
          top_left_y = 1;
          bottom_right_y = pad_tmp_data.height_ - 2;
        }
        bool ret = Crop(pad_tmp_data,
                        top_left_x,
                        top_left_y,
                        bottom_right_x,
                        bottom_right_y,
                        output);
        HobotXStreamFreeImage(pad_tmp_data.data_[0]);
        return ret;
      } else {

 ...
 public:
  ...
  void OnCallback(xstream::OutputDataPtr output) {
    ...
    LOGD << "[MultisourceTest]:source_id " << output->source_id_
         << " sequence_id " << output->sequence_id_ << " frame_id "
         << out_test->frame_id_ << " method_id " << std::dec
         << out_test->method_id_ << " thread_hash " << std::hex
         << out_test->thread_hash_;
    resbuffer.push_back(out_test);
  }

  int ExamResult(xstream::MethodInfo methodinfo, size_t source_num,
                 size_t thread_num) {
    ...
    for (auto outdata : resbuffer) {
      auto method_id = outdata->method_id_;
      // auto frame_id = outdata->value.frame_id_;
      auto thread_hash = outdata->thread_hash_;
      auto source_id = outdata->source_id_;
      if (method_list.find(outdata->method_id_) == method_list.end())
        method_list.insert(outdata->method_id_);
      if (thread_list.find(outdata->thread_hash_) == thread_list.end())
        thread_list.insert(outdata->thread_hash_);
      if (source_list.find(outdata->source_id_) == thread_list.end())
        source_list.insert(outdata->source_id_);
    ...
    if (source_num > 0 && source_list.size() != source_num) {
      std::cout << "expect number " << source_list.size()
                << " of soure_id equal to source_num " << source_num
                << std::endl;
      return -1;
    }
    // 检查收到数据是否包含所有thread
      ...
      for (auto methodmap : thread_to_method) {
        for (auto methodid : methodmap.second) {
          if (templist.find(methodid) != templist.end()) {
            ...
            std::cout << "is_thread_safe_ == false, methodid " << methodid
                      << " binded more than one thread" << std::endl;
            return -1;
          }
          templist.insert(methodid);
        }
      }
    } else if (!methodinfo.is_src_ctx_dept_ && thread_num != 1) {
    ...
    if (methodinfo.is_src_ctx_dept_ == true) {
      if (method_list.size() != source_num) {
        std::cout << "is_src_ctx_dept_==true, total method istance "
                  << method_list.size() << " should be equal to source num "
                  << source_num << std::endl;
        return -1;
      }
      if (thread_list.size() > source_list.size()) {
        std::cout << "is_src_ctx_dept_==true, thread num " << thread_list.size()
                  << " can not be larger than source_num "
                  << source_list.size();
      }
      ...
      for (auto sourceid : source_list) {
        if (source_to_method[sourceid].size() != 1) {
          std::cout << "is_src_ctx_dept_==true, source id " << sourceid
                    << " binded more than one method" << std::endl;
          return -1;
        }
        ...
        if (templist.find(*source_to_method[sourceid].begin()) !=
          ...
          std::cout << "is_src_ctx_dept_==true, method id  "
      ...
      if (methodinfo.is_thread_safe_ == false) {
        ...
        for (auto methodlist : thread_to_method) {
          if (methodlist.second.size() != avg &&
            ...
            std::cout << "is_thread_safe_ = false，is_src_ctx_dept_==true，"
      ...
      // is_src_ctx_dept_ == false
      if (methodinfo.is_thread_safe_ == false &&
          method_list.size() != thread_num) {
        std::cout << "When is_src_ctx_dept_ = false, is_thread_safe_ = false"
                  << "method number " << method_list.size() << "shall equal to"
                  << "thread_num " << thread_num << std::endl;
        return -1;
      }
      if (methodinfo.is_thread_safe_ == true && method_list.size() != 1) {
        std::cout << "When is_src_ctx_dept_ = false, is_thread_safe_ = true"
                  << "method number " << method_list.size()
                  << "shall equal to 1" << std::endl;
        return -1;
      }
    }
...
int DoTest(const std::string &config_template, const std::string &config_temp,
  ...
  for (auto source_id : shuffled_sourceid) {
    ...
    test_input->source_id_ = source_id;
    test_input->frame_id_ = frame_id[source_id]++;
...
static const char config_template[] = "./test/configs/multisource_test.json";
...
TEST(MultiSourceTestMethod, BackwardCompatible1) {
  ...
  methodinfo.is_src_ctx_dept_ = false;

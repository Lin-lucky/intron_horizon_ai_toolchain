...
TEST(Interface, PassThrough) {
  ...
  xstream::BBox *bbox1(new xstream::BBox(xstream::BBox(0, 0, 60, 60)));
  bbox1->type_ = "BBox";
...
TEST(Interface, UsePreDefine) {
  ...
  xstream::BBox *pre_bbox1(new xstream::BBox(xstream::BBox(0, 0, 70, 70)));
  ...
  for (auto data : out->datas_) {
    ...
    for (auto &in_rect : in_rects->datas_) {
      ...
      EXPECT_EQ(bbox->x2_, 70);
      EXPECT_EQ(bbox->y2_, 70);
      std::cout << "output face_head_box: " << bbox->x1_ << "," << bbox->y1_
                << "," << bbox->x2_ << "," << bbox->y2_ << std::endl;
    }
  }
...
TEST(Interface, Disable) {
  ...
  NodeTest::Callback callback;
  EXPECT_EQ(
      0, xstream->SetConfig("config_file",
                            "./test/configs/bbox_filter_disable_method.json"));
  EXPECT_EQ(0, xstream->Init());
  InputDataPtr inputdata(new InputData());
  BaseDataVector *data(new BaseDataVector);
  xstream::BBox *bbox1(new xstream::BBox(xstream::BBox(0, 0, 80, 80)));
  bbox1->type_ = "BBox";
  data->name_ = "face_head_box";
  data->datas_.push_back(BaseDataPtr(bbox1));
  inputdata->datas_.push_back(BaseDataPtr(data));

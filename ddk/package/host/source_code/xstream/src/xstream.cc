...
int XStreamFlow::Init(MethodFactoryPtr method_factory) {
  ...
  if (param_dict_.count("config_string") > 0) {
    ret = config->LoadJsonString(param_dict_["config_string"]);
  } else {
    ret = config->LoadFile(config_file_);
  }
  if (ret != 0) {
    LOGE << "Load workflow config failed";
    return -1;
  }
...
int XStreamFlow::SetConfig(const std::string &key, const std::string &value) {
  ...
  } else if (key.compare("config_string") == 0) {
    param_dict_["config_string"] = value;
...
OutputDataPtr XStreamFlow::SyncPredict(InputDataPtr input) {
  if (!input || input->datas_.size() <= 0) {
    return OnError(XSTREAM_ERROR_INPUT_INVALID, "input error");
  }
...
std::vector<OutputDataPtr> XStreamFlow::SyncPredict2(InputDataPtr input) {
  if (!input || input->datas_.size() <= 0) {
    OnError(XSTREAM_ERROR_INPUT_INVALID, "input error");
  }
  std::promise<std::vector<OutputDataPtr>> promise;
  auto future = promise.get_future();
...
int64_t XStreamFlow::GetTaskNum() { return scheduler_->GetTaskNum(); }
}  // namespace xstream

...
int XStreamConfig::LoadJsonString(const std::string& json_string) {
  Json::CharReaderBuilder reader_builder;
  std::unique_ptr<Json::CharReader> const json_reader\
          (reader_builder.newCharReader());
  JSONCPP_STRING json_err;
  bool parse_ret = json_reader->parse(json_string.c_str(),
                              json_string.c_str() + json_string.length(),
                              &cfg_jv_, &json_err);
  if (!parse_ret) {
    LOGE << "Load workflow json string failed: "
         << json_err;
    return -1;
  }
  folder_path_ = "./";  // use current path
  // version
  if (cfg_jv_.isMember("version")) {  // parse config
    Config out;
    parse_config_.folder_path_ = folder_path_;

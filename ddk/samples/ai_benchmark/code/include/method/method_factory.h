...
#define DEFINE_AND_REGISTER_METHOD(method_name)                              \
  xstream::SimpleMethod *method_name##_creator() { return new method_name; } \
  static MethodRegistry method_registry(#method_name, method_name##_creator);

typedef xstream::SimpleMethod *(*MethodCreator)();
...
class MethodFactory {
 public:
  ...
  xstream::SimpleMethod *GetMethod(const std::string &method_name) {

...
#include "method/ptq_yolo3_varg_darknet_post_process_method.h"
...
DEFINE_AND_REGISTER_METHOD(PTQYolo3VargDarknetPostProcessMethod);

PTQYolo3VargDarknetConfig default_ptq_yolo3_varg_darknet_config = {
    {8, 16, 32},
...
int PTQYolo3VargDarknetPostProcessMethod::Init(
    const std::string &config_file_path) {
  VLOG(EXAMPLE_DEBUG) << "PTQYolo3VargDarknetPostProcessMethod init from file";
...
int PTQYolo3VargDarknetPostProcessMethod::InitFromJsonString(
    const std::string &config) {
  VLOG(EXAMPLE_DEBUG) << "PTQYolo3VargDarknetPostProcessMethod Json string:"
                      << config.data();
...
std::vector<xstream::BaseDataPtr>
PTQYolo3VargDarknetPostProcessMethod::DoProcess(
  ...
  VLOG(EXAMPLE_DETAIL) << "PTQYolo3VargDarknetPostProcessMethod DoProcess "
                          "finished, predict result: "
                       << *(perception.get());
  return res;
}

void PTQYolo3VargDarknetPostProcessMethod::Finalize() {}

int PTQYolo3VargDarknetPostProcessMethod::UpdateParameter(
    xstream::InputParamPtr ptr) {
  VLOG(EXAMPLE_REPORT)
      << "UpdateParameter in PTQYolo3VargDarknetPostProcessMethod not "
         "supported yet.";
  return 0;
}

xstream::InputParamPtr PTQYolo3VargDarknetPostProcessMethod::GetParameter()
    const {
  VLOG(EXAMPLE_DETAIL) << "PTQYolo3VargDarknetPostProcessMethod GetParameter";
  xstream::InputParamPtr ptr;
  return ptr;
}

std::string PTQYolo3VargDarknetPostProcessMethod::GetVersion() const {}

xstream::MethodInfo PTQYolo3VargDarknetPostProcessMethod::GetMethodInfo() {
  VLOG(EXAMPLE_DETAIL) << "PTQYolo3VargDarknetPostProcessMethod GetMethodInfo";
...
PTQYolo3VargDarknetPostProcessMethod::~PTQYolo3VargDarknetPostProcessMethod() {}

int PTQYolo3VargDarknetPostProcessMethod::PostProcess(
...
void PTQYolo3VargDarknetPostProcessMethod::PostProcessNHWC(
    hbDNNTensor *tensor,
    ImageTensor *frame,
    int layer,
    std::vector<Detection> &dets) {
  ...
  double resize_ratio_h = frame->height() * 1.0 / frame->ori_height();
  double resize_ratio_w = frame->width() * 1.0 / frame->ori_width();
  ...
  for (uint32_t h = 0; h < height; h++) {
    for (uint32_t w = 0; w < width; w++) {
      for (int k = 0; k < anchors.size(); k++) {
        ...
        double xmin_org = xmin / resize_ratio_w;
        double xmax_org = xmax / resize_ratio_w;
        double ymin_org = ymin / resize_ratio_h;
        double ymax_org = ymax / resize_ratio_h;
...
void PTQYolo3VargDarknetPostProcessMethod::PostProcessNCHW(
    hbDNNTensor *tensor,
    ImageTensor *frame,
    int layer,
    std::vector<Detection> &dets) {

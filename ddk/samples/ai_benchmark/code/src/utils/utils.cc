...
hbDNNTensorShape squeeze(hbDNNTensorShape &shape) {
  hbDNNTensorShape s;
  s.numDimensions = 0;
  for (int i = 0; i < shape.numDimensions; i++) {
    if (shape.dimensionSize[i] != 1) {
      s.dimensionSize[s.numDimensions++] = shape.dimensionSize[i];
    }
  }
  return s;
}
...
void SetLatency(Latency &latency) {
  latency.count++;
  gettimeofday(&latency.start_time, NULL);
}

void UpdateLatency(Latency &latency, const std::string &s) {
  gettimeofday(&latency.end_time, NULL);
  double time_elapsed =
      ((latency.end_time.tv_sec - latency.start_time.tv_sec) * 1000000 +
       (latency.end_time.tv_usec - latency.start_time.tv_usec)) /
      1000.0;

  if (time_elapsed > latency.max_time) latency.max_time = time_elapsed;
  if (time_elapsed < latency.min_time) latency.min_time = time_elapsed;

  latency.total_time += time_elapsed;
  if (latency.count % 10 == 0) {
    VLOG(EXAMPLE_REPORT) << std::fixed << std::setprecision(3)
                         << RED_COMMENT_START << s << " avg latency:  "
                         << (latency.total_time - latency.min_time -
                             latency.max_time) /
                                (latency.count - 2)
                         << "ms  max:  " << latency.max_time
                         << "ms  min:  " << latency.min_time << "ms"
                         << RED_COMMENT_END;
  }
}

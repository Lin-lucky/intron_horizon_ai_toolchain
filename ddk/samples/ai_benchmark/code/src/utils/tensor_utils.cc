...
int read_image_cache(std::string &path,
                     hbDNNTensor *tensor,
                     ImageCache *cache) {
  if (tensor->properties.tensorType == HB_DNN_IMG_TYPE_YUV444) {
    int h_idx, w_idx, c_idx;
    ...
    auto ori_width = bgr_mat.cols;
    auto ori_height = bgr_mat.rows;
    mat.create(height, width, bgr_mat.type());
    cv::resize(bgr_mat, mat, mat.size(), 0, 0);

    // cover to yuv444
    cv::Mat yuv_mat;
    cv::cvtColor(mat, yuv_mat, cv::COLOR_BGR2YUV);

    auto *buffer = new uint8_t[height * width * 3];
    memcpy(buffer, yuv_mat.data, height * width * 3);
    ...
    // insert in cache
    if (nullptr == cache ||
        !cache->AddImage(path, buffer, width, height, ori_width, ori_height)) {
      delete buffer;
    }
  } else if (tensor->properties.tensorType == HB_DNN_IMG_TYPE_NV12) {
  ...
  } else if (tensor->properties.tensorType == HB_DNN_IMG_TYPE_Y) {
    int h_idx, w_idx, c_idx;
    get_tensor_hwc_index(tensor, &h_idx, &w_idx, &c_idx);
    ...
    auto *buffer = new uint8_t[height * width];

    memcpy(buffer, data, height * width);
    // insert in cache
    if (nullptr == cache ||
        !cache->AddImage(path, buffer, width, height, ori_width, ori_height)) {
      delete buffer;
    ...
    VLOG(EXAMPLE_SYSTEM)
        << "read image cache failed: un support image data type "
        << data_type_enum_to_string(tensor->properties.tensorType);
    return -1;
  }
  return 0;
}
...
int read_image_tensor(std::string &path,
  ...
  // find in cache
  if (nullptr != cache &&
      cache->FindImage(path, &buffer, ori_width, ori_height)) {
    if (data_type == HB_DNN_IMG_TYPE_NV12) {
      memcpy(tensor->sysMem[0].virAddr, buffer, height * width * 3 / 2);
    } else if (data_type == HB_DNN_IMG_TYPE_YUV444) {
      memcpy(tensor->sysMem[0].virAddr, buffer, height * width * 3);
    } else if (data_type == HB_DNN_IMG_TYPE_Y) {
      memcpy(tensor->sysMem[0].virAddr, buffer, height * width);
    } else {
      VLOG(EXAMPLE_SYSTEM) << "unsupport read image from cache, data type is "

...
DEFINE_int32(data_size, 802816, "Data size, 802816 by default");
...
/**
 * Prepare tensor and fill with feature data
 * @param[out] input_tensor: tensor to be prepared
 * @param[in] dnn_handle: dnn handle
 * @param[in] data_size: data size
 * @return 0 if success otherwise -1
 */
int prepare_feature_tensor(hbDNNTensor *input_tensor,
...
int main(int argc, char **argv) {
  ...
  hbDNNTensor input;
  HB_CHECK_SUCCESS(
      prepare_feature_tensor(&input, dnn_handle, data, data_length),
      "prepare feature tensor failed");
  VLOG(EXAMPLE_DEBUG) << "prepare feature tensor success";
...
int prepare_feature_tensor(hbDNNTensor *input_tensor,
  ...
  HB_CHECK_SUCCESS(hbSysAllocMem(&mem, data_size),
                   "hbSysAllocCachedMem failed");
  memcpy(mem.virAddr, data, data_size);
  return 0;
}
...
int prepare_tensor(hbDNNTensor **output_tensor,
  ...
  for (int i = 0; i < output_count; i++) {
    ...
    int out_aligned_size = output_properties.alignedByteSize;  // S32

...
def postprocess(model_output,
                ...
                nms_threshold=0.45,
                dump_image=True):
    yolov2_config = get_yolov2_config()
    anchors = yolov2_config.ANCHORS
    ...
    pred_bbox = model_output[0]
    pred_bbox = pred_bbox.transpose([0, 3, 1, 2])
    pred_bbox = yolo_decoder(pred_bbox, num_anchors, num_classes, anchors,
                             yolov2_config.STRIDES)
...
def calc_accuracy(annotation_path, accuracy):
    ...
    total_samples = 0
    with open("yolov2_eval.log", "w") as eval_log_handle:
...
def get_yolov2_config():
    ...
    yolov2_config.NUM_CLASSES = 80
    yolov2_config.CLASSES = MSCOCODetMetric.class_names
    yolov2_config.INPUT_SHAPE = (608, 608)
    return yolov2_config

...
class MeanTransformer(Transformer):
    ...
    def run_transform(self, data):
        if self.data_format is "HWC":
            data[0] = data[0] - self.means
        else:
            data[0] = data[0] - self.means[:, np.newaxis, np.newaxis]
        data[0] = data[0].astype(np.float32)
        return data
...
class ScaleTransformer(Transformer):
    ...
    def run_transform(self, data):
        data[0] = data[0] * self.scale_value
        data[0] = data[0].astype(np.float32)
        return data
...
class CenterCropTransformer(Transformer):
    ...
    def run_transform(self, data):
        ...
        data[0] = image[resize_up:resize_up + self.crop_size,
...
class PILCenterCropTransformer(Transformer):
    ...
    def run_transform(self, data):
        img = Image.fromarray(data[0].astype('uint8'), 'RGB')
...
class ResizeTransformer(Transformer):
    ...
    # 4: Bi-quartic
    # 5: Bi-quintic
    def __init__(self, target_size, mode='skimage', method=1):
        self.target_size = target_size
    ...
    def opencv_resize(self, data):
        data[0] = cv2.resize(data[0], self.target_size)
        data[0] = data[0].astype(np.float32)
        return data

    def skimage_resize(self, data):
        """
        im : (H x W x K)
        ...
        data[0] = resized_im.astype(np.float32)
        data[0] = data[0].astype(np.float32)
        return data
    ...
    def run_transform(self, data):
        ...
        if isinstance(self.size, int):
            ...
            if w < h:
                ...
                data[0] = np.array(img.resize((ow, oh), self.interpolation))
                return data
            else:
                oh = self.size
            ...
            data[0] = np.array(img.resize(self.size[::-1], self.interpolation))
            return data
...
class ShortLongResizeTransformer(Transformer):
    ...
    def run_transform(self, data):
        ...
        im_scale = float(self.short_size) / float(im_size_min)
        if round(im_scale * im_size_max) > self.long_size:
            im_scale = float(self.long_size) / float(im_size_max)
    ...
    def run_transform(self, data):
        ...
        if height < width:
            ...
            image = cv2.resize(image,
                               (int(self.short_size * off), self.short_size))
        else:
            off = height / width
            image = cv2.resize(image,
                               (self.short_size, int(self.short_size * off)))
        data[0] = image
        data[0] = data[0].astype(np.float32)
        return data
...
class PaddedCenterCropTransformer(Transformer):
    ...
    def run_transform(self, data):
        ...
        data[0] = image[offset_height:offset_height + padded_center_crop_size,
...
class _ColorConvertTransformer(Transformer):
    ...
    def run_transform(self, data):
        ...
        data[0] = converted_image.astype(np.float32)
        return data
...
class YUVTransformer(Transformer):
    ...
    def run_transform(self, data):
        data[0] = self._py_func(data[0], self.color_sequence == "RGB")
        return data
...
class BGR2NV12Transformer(Transformer):
    ...
    def run_transform(self, data):
        ...
        if self.cvt_mode == 'opencv':
            data[0] = self.rgb2nv12_opencv(image)
        else:
            data[0] = self.rgb2nv12_calc(image)
        return data
...
class WarpAffineTransformer(Transformer):
    ...
    def run_transform(self, data):
        ...
        inp_image = cv2.warpAffine(data[0],
                                   trans_input,
                                   (self.input_width, self.input_height),
                                   flags=cv2.INTER_LINEAR)
        data[0] = inp_image.astype(np.float32)

...
#include "filter_method/filter_data_type.h"
  ...
  for (int i = 0; i < input_size; i++) {
    ...
    if ("id" == data_info.type) {
      for (size_t j = 0; j < data_size; j++) {
        auto lost_value = std::static_pointer_cast<xstream::XStreamID>(
            input_slot[i]->datas_[j]);
...
int FilterMethod::GetIdByIdx(const int &input_size,
  ...
  for (int i = 0; i < input_size; i++) {
    ...
    if ("bbox" == data_info.type) {
      auto bbox = std::static_pointer_cast<xstream::BBox>(
          input_slot[i]->datas_[box_idx]);
  ...
  for (int box_idx = 0; box_idx < box_size; ++box_idx) {
    ...
    if (error_code == filter_param_->passed_err_code) {
      ...
      auto xstream_description =
          dynamic_cast<xstream::XStreamFilterDescription *>(description.get());
  ...
  if (filter_param_->max_face_box_counts) {
    ...
    std::partial_sort(
        ...
        sorted_indexs.end(), [&output_slot, &i](size_t lhs, size_t rhs) {
          auto face_box_lhs = std::static_pointer_cast<xstream::BBox>(
              output_slot[i + 1]->datas_[lhs]);
          auto face_box_rhs = std::static_pointer_cast<xstream::BBox>(
              output_slot[i + 1]->datas_[rhs]);
          return face_box_lhs->Width() * face_box_lhs->Height() >
                 face_box_rhs->Width() * face_box_rhs->Height();
        });
...
int FilterMethod::FaceValid(const int &input_size,
  ...
  for (int i = 0; i < input_size; i++) {
    ...
    if ("bbox" == data_info.type) {
      auto bbox =
          std::static_pointer_cast<xstream::BBox>(input_slot[i]->datas_[idx]);
      if (bbox->state_ == DataState::VALID) {
        float x1 = bbox->x1_;
        float y1 = bbox->y1_;
        float x2 = bbox->x2_;
        float y2 = bbox->y2_;
        float box_score = bbox->score_;
        int32_t id = bbox->id_;
        id_pass = !MatchStopId(id);
        size_pass =
            ReachSizeThreshold(x1, y1, x2, y2, filter_param_->face_size_thr);
        expand_pass =
            ExpandThreshold(bbox.get(), filter_param_->face_expand_scale);
        score_pass =
      ...
      auto pose =
          std::static_pointer_cast<xstream::Pose3D>(input_slot[i]->datas_[idx]);
      ...
      auto age =
          std::static_pointer_cast<xstream::Age>(input_slot[i]->datas_[idx]);
      if (age->state_ == DataState::VALID) {
        age_pass =
            ReachAgeThreshold(age->value_, age->min_, age->max_, age->score_);
        if (!age_pass) {
          FILTER_LOG_VALUE(idx, data_info.name, age->value_)
          FILTER_LOG_VALUE(idx, data_info.name, age->score_)
      ...
      if (attribute->state_ == DataState::VALID) {
        upper_score = attribute->score_;
        upper_limit_pass = BelowUpperLimit(attribute->score_, limit_val);
        if (!upper_limit_pass) {
      ...
      if (attribute->state_ == DataState::VALID) {
        lower_score = attribute->score_;
        lower_limit_pass = AboveLowerLimit(attribute->score_, limit_val);
        if (!lower_limit_pass) {
  ...
  for (int i = 0; i < input_size; i++) {
    ...
    if ("bbox" == data_info.type) {
      ...
      if (bbox->state_ == DataState::VALID) {
        ...
        expand_pass =
            ExpandThreshold(bbox.get(), filter_param_->head_expand_scale);
        score_pass =
  ...
  for (int i = 0; i < input_size; i++) {
    ...
    if ("bbox" == data_info.type) {
      ...
      if (bbox->state_ == DataState::VALID) {
        ...
        expand_pass =
            ExpandThreshold(bbox.get(), filter_param_->body_expand_scale);
        score_pass =
  ...
  for (int i = 0; i < input_size; i++) {
    ...
    if ("bbox" == data_info.type) {
      ...
      if (bbox->state_ == DataState::VALID) {
        ...
        expand_pass =
            ExpandThreshold(bbox.get(), filter_param_->hand_expand_scale);
        score_pass =
...
BaseDataVectorPtr FilterMethod::CopySlot(const BaseDataVectorPtr &attr,
  ...
  for (const auto &data : attr->datas_) {
    if ("bbox" == data_type) {
      auto actual_data = std::static_pointer_cast<xstream::BBox>(data);
      std::shared_ptr<xstream::BBox> copy_data =
      ...
      *copy_data = *actual_data;
      ...
      auto actual_data = std::static_pointer_cast<xstream::Pose3D>(data);
      std::shared_ptr<xstream::Pose3D> copy_data =
          std::make_shared<xstream::Pose3D>();
      *copy_data = *actual_data;
      out->datas_.push_back(std::static_pointer_cast<BaseData>(copy_data));
    } else if ("landmark" == data_type) {
      auto actual_data = std::static_pointer_cast<xstream::Landmarks>(data);
      std::shared_ptr<xstream::Landmarks> copy_data =
          std::make_shared<xstream::Landmarks>();
      *copy_data = *actual_data;
      out->datas_.push_back(std::static_pointer_cast<BaseData>(copy_data));
    } else if ("id" == data_type) {
      auto actual_data = std::static_pointer_cast<xstream::XStreamID>(data);
      std::shared_ptr<xstream::XStreamID> copy_data =
          std::make_shared<xstream::XStreamID>();
      *copy_data = *actual_data;
      out->datas_.push_back(std::static_pointer_cast<BaseData>(copy_data));
    } else if ("age" == data_type) {
      auto actual_data = std::static_pointer_cast<xstream::Age>(data);
      std::shared_ptr<xstream::Age> copy_data =
          std::make_shared<xstream::Age>();
      *copy_data = *actual_data;
      out->datas_.push_back(std::static_pointer_cast<BaseData>(copy_data));
...
bool FilterMethod::LmkVerification(
    const std::shared_ptr<xstream::Landmarks> &lmk) {
  if (filter_param_->lmk_filter_num > 0) {
    int filter_num = 0;
    for (const auto &point : lmk->values_) {
      LOGD << "point.score:" << point.score_;
      LOGD << "filter_lmk_thr:" << filter_param_->lmk_thr;
      if (point.score_ < filter_param_->lmk_thr) {
...
bool FilterMethod::ExpandThreshold(xstream::BBox *box,
...
BaseDataVectorPtr FilterMethod::ConstructBoundOutputSlot() {
  BaseDataVectorPtr out = std::make_shared<BaseDataVector>();
  auto bound_box = std::make_shared<xstream::BBox>();
  bound_box->x1_ = filter_param_->bound_thr_w;
  bound_box->y1_ = filter_param_->bound_thr_h;
  bound_box->x2_ = filter_param_->image_width - filter_param_->bound_thr_w;
  bound_box->y2_ = filter_param_->image_height - filter_param_->bound_thr_h;
  bound_box->id_ = 1;
  bound_box->score_ = 1.0;
  out->datas_.emplace_back(bound_box);
  auto show_rect = std::make_shared<xstream::Attribute_<bool>>();
  show_rect->value_ = filter_param_->show_bound_rect;
  LOGD << "show_rect: " << show_rect->value_;
...
BaseDataVectorPtr FilterMethod::ConstructFilterOutputSlot0(
  ...
  for (size_t i = 0; i < face_num; i++) {
    auto data = std::make_shared<xstream::XStreamFilterDescription>();
...
int FilterMethod::NormalizeRoi(xstream::BBox *src, float norm_ratio,
                               NormMethod norm_method, uint32_t total_w,
                               uint32_t total_h) {
  if (std::fabs(norm_ratio - 1) < 0.00001) return 0;
  xstream::BBox dst = *src;

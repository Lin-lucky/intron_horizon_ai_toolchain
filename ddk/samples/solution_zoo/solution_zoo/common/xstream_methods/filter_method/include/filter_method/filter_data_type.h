...
#ifndef COMMON_XSTREAM_METHODS_FILTER_METHOD_\
INCLUDE_FILTER_METHOD_FILTER_DATA_TYPE_H_
#define COMMON_XSTREAM_METHODS_FILTER_METHOD_\
INCLUDE_FILTER_METHOD_FILTER_DATA_TYPE_H_
...
struct XStreamID : public BaseData {
  uint32_t value;
  XStreamID() {}
  explicit XStreamID(const uint32_t &val) { value = val; }
};

struct XStreamFilterDescription : public BaseData {
  uint32_t value;
  XStreamFilterDescription() {}
  explicit XStreamFilterDescription(const uint32_t &val) { value = val; }
};

struct Area : xstream::BBox {
...
class WhiteListsModule : public ListsModule {
 public:
  bool IsInArea(float x1, float y1, float x2, float y2,
    ...
    float c_x = (x1 + x2) / 2;
    float c_y = (y1 + y2) / 2;
    if ((c_x >= white_area.x1_) && (c_x <= white_area.x2_) &&
        (c_y >= white_area.y1_) && (c_y <= white_area.y2_)) {
      return true;
    } else {
      return false;
    }
...
#endif  // COMMON_XSTREAM_METHODS_FILTER_METHOD_INCLUDE_FILTER_METHOD_FILTER_DATA_TYPE_H_

...
INCLUDE_FILTER_METHOD_FILTER_METHOD_H_
...
class FilterMethod : public SimpleMethod {
 ...
 private:
  ...
  bool LmkVerification(const std::shared_ptr<xstream::Landmarks> &lmk);
  ...
  bool IsWithinWhiteListArea(const float &x1, const float &y1, const float &x2,
                             const float &y2);
  //  filter by expand
  bool ExpandThreshold(xstream::BBox *box, const float &expand_scale);
 ...
 private:
  ...
  int NormalizeRoi(xstream::BBox *src, float norm_ratio, NormMethod norm_method,
...
#endif  // COMMON_XSTREAM_METHODS_FILTER_METHOD_INCLUDE_FILTER_METHOD_FILTER_METHOD_H_

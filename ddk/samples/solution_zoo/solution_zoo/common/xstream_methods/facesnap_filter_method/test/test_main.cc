 ...
 protected:
  virtual void SetUp() {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      brightness_.push_back(std::make_shared<Attribute>());
      brightness_[i]->score_ = 0.9;
      brightness_[i]->value_ = 0;
      brightness_[i]->type_ = "Attribute";
      occluded_attribute_.push_back(std::make_shared<Attribute>());
  ...
  std::vector<std::shared_ptr<Attribute>> occluded_attribute_;
  std::vector<std::shared_ptr<Attribute>> brightness_;
};
...
TEST_F(FaceSnapFilterMethodTest, default_config) {
  ...
  for (size_t i = 0; i < output_box->datas_.size(); i++) {
    ...
    if (data->state_ == DataState::VALID) {
      ...
      auto err_code = std::static_pointer_cast<Attribute>(
          output_err_code->datas_[i]);
      EXPECT_EQ(err_code->value_, -4);
    }
  }
...
TEST_F(FaceSnapFilterMethodTest, big_face_mode) {
  ...
  EXPECT_EQ(err_code->value_, -8);
  err_code = std::static_pointer_cast<Attribute>(
      output_err_code->datas_[1]);
  EXPECT_EQ(err_code->value_, 0);
  auto output_box = std::static_pointer_cast<BaseDataVector>(output[1]);
  EXPECT_EQ(output_box->datas_.size(), static_cast<size_t>(2));
  actual_size = 0;

...
std::set<int> HeadFaceStrategy::GetIdSet(const BaseDataPtr &boxPtr) {
  ...
  for (const auto &data : box_list->datas_) {
    if (data->state_ == xstream::DataState::VALID) {
      auto box = std::static_pointer_cast<BBox>(data);
      auto &id = box->id_;
...
void HeadFaceStrategy::GetMainId(
  ...
  for (const auto &data : box_list->datas_) {
    auto pdata = std::static_pointer_cast<BBox>(data);
...
std::vector<std::pair<int, int>> HeadFaceStrategy::GetMatchedPairs(
  ...
  for (const auto &data : face_box_list->datas_) {
    auto bbox = std::static_pointer_cast<BBox>(data);
    auto area = bbox->Height() * bbox->Width();
  ...
  for (size_t head_track_i = 0; head_track_i < head_bbox_num; ++head_track_i) {
    ...
    if (head_bbox->state_ == xstream::DataState::VALID) {
      auto &head_track_id = head_box->id_;
      int max_ind = -1;
      float max_ratio = 0;
      BBox inter_box;
      for (size_t face_track_i = 0; face_track_i < face_bbox_num;
        ...
        if (face_data->state_ == xstream::DataState::VALID) {
          ...
          inter_box.x1_ = (std::max)(
              face_box->x1_,
              MoveCoord(head_box->x1_, box_offset_.x_offset_ratio,
                        head_box->Width()));
          inter_box.x2_ = (std::min)(
              face_box->x2_,
              MoveCoord(head_box->x2_, box_offset_.x_offset_ratio,
                        head_box->Width()));
          inter_box.y1_ = (std::max)(
              face_box->y1_,
              MoveCoord(head_box->y1_, box_offset_.y_offset_ratio,
                        head_box->Height()));
          inter_box.y2_ = (std::min)(
              face_box->y2_,
              MoveCoord(head_box->y2_, box_offset_.y_offset_ratio,
                        head_box->Height()));

          if (inter_box.x2_ <= inter_box.x1_ ||
              inter_box.y2_ <= inter_box.y1_) {
            continue;  // no intersection
          }
          auto area =
              (inter_box.Width() + 1) * (inter_box.Height() + 1);
      ...
      if (max_ratio > threshold && max_ind != -1) {
        ...
        if (face_data->state_ == xstream::DataState::VALID) {
          auto face_box = std::static_pointer_cast<BBox>(face_data);
          auto face_track_id = face_box->id_;

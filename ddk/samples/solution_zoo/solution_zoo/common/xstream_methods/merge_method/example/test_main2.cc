...
BaseDataVectorPtr parsing_input_str_stream(const std::string &info) {
  ...
  while (ss >> x1 >> y1 >> x2 >> y2 >> track_id) {
    auto box1 = std::make_shared<BBox>();
    box1->type_ = "BBox";
    box1->id_ = track_id;
    box1->x1_ = x1;
    box1->y1_ = y1;
...
BoxKpsList parsing_person_det_stream(const std::string &info) {
  ...
  while (ss >> x1 >> y1 >> x2 >> y2 >> track_id) {
    ...
    int kp_x, kp_y;
    float kp_score;
    auto body_kps1 = std::make_shared<Landmarks>();
    body_kps1->type_ = "Kps";
    body_kps1->values_.resize(17);

    for (size_t kp_idx = 0; kp_idx < 17; ++kp_idx) {
      ss >> kp_x >> kp_y >> kp_score;
      body_kps1->values_[kp_idx].x_ = kp_x;
      body_kps1->values_[kp_idx].y_ = kp_y;
      body_kps1->values_[kp_idx].score_ = kp_score;
    }
...
int main(int argc, char const *argv[]) {
  ...
  while (getline(fid_in, info) && getline(face_fid_in, face_info) &&
    ...
    head_id->value_ = rand_r(&seed) % 1000;
    LOGD << "lost head id: " << head_id->value_;
    ...
    face_id->value_ = rand_r(&seed) % 1000;
    LOGD << "lost face id: " << face_id->value_;
    ...
    body_id->value_ = rand_r(&seed) % 1000;
    LOGD << "lost body id: " << body_id->value_;
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (pdata->name_ == "merged_face_box") {
        for (const auto &element : pdata->datas_) {
          ...
          LOGD << "face " << bbox->x1_ << " " << bbox->y1_ << " "
               << bbox->x2_ << " " << bbox->y2_ << " "
               << bbox->id_;
          face_res_f << " " << bbox->x1_ << " " << bbox->y1_ << " "
                     << bbox->x2_ << " " << bbox->y2_ << " "
                     << bbox->id_;
        }
        ...
        for (const auto &element : pdata->datas_) {
          auto bbox = std::static_pointer_cast<BBox>(element);
          LOGD << "body " << bbox->x1_ << " " << bbox->y1_ << " "
               << bbox->x2_ << " " << bbox->y2_ << " "
               << bbox->id_;
          body_res_f << " " << bbox->x1_ << " " << bbox->y1_ << " "
                     << bbox->x2_ << " " << bbox->y2_ << " "
                     << bbox->id_;
        }

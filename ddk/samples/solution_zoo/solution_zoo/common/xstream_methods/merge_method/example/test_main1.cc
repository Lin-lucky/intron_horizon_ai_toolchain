...
void Dump(const xstream::OutputDataPtr &output) {
  ...
  if (!pdata->datas_.empty()) {
    for (const auto &element : pdata->datas_) {
      ...
      disappeared_id.emplace_back(num->value_);
    }
  }
  if (!face_id.empty()) {
    std::cout << " face:";
  ...
  if (in) {
    while (getline(in, file)) {
      ...
      for (auto i = 0; i < size; ++i) {
        ...
        box->type_ = "BBox";
        box->id_ = atof(elements[i * 6 + 0].c_str());
        box->x1_ = atof(elements[i * 6 + 2].c_str());
        box->y1_ = atof(elements[i * 6 + 3].c_str());
        box->x2_ = atof(elements[i * 6 + 4].c_str());
        box->y2_ = atof(elements[i * 6 + 5].c_str());

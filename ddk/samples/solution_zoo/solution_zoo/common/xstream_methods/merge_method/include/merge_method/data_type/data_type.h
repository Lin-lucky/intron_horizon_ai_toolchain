...
inline void DumpBox(const std::shared_ptr<BBox> &box) {
  std::cout << box->x1_ << std::endl;
  std::cout << box->y1_ << std::endl;
  std::cout << box->x2_ << std::endl;
  std::cout << box->y2_ << std::endl;
}

inline float intersection(const BBox &box1, const BBox &box2) {
  auto x1 = (std::max)(box1.x1_, box2.x1_);
  auto y1 = (std::max)(box1.y1_, box2.y1_);
  auto x2 = (std::min)(box1.x2_, box2.x2_);
  auto y2 = (std::min)(box1.y2_, box2.y2_);
  if (x1 >= x2 || y1 >= y2) {
    return 0;
  }

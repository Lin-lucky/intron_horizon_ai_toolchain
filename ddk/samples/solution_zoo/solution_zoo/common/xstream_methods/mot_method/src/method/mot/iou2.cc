...
typedef ImageFramePtr XStreamImageFrame;
...
int IOU2::Track(const std::vector<BaseDataPtr> &in,
  ...
  HOBOT_CHECK("PyramidImageFrame" == img_frame->type_);
  HOBOT_CHECK("BaseDataVector" == box_list->type_);

//  auto pframe = std::static_pointer_cast<XStreamImageFrame>(img_frame);
  auto pframe = std::static_pointer_cast<PyramidImageFrame>(img_frame);
  //  time_t time_stamp = pframe->value->time_stamp/1000;
  time_t time_stamp = pframe->time_stamp_;
  uint32_t frame_width = pframe->Width(0);
  uint32_t frame_height = pframe->Height(0);
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    hobot::iou_mot::sp_BBox iou_bbox =
      std::make_shared<hobot::iou_mot::BBox_s>(
           std::max(0, static_cast<int>(std::lround(bbox->x1_))),
           std::max(0, static_cast<int>(std::lround(bbox->y1_))),
           std::min(img_width - 1,
                    static_cast<int>(std::lround(bbox->x2_))),
           std::min(img_height - 1,
                    static_cast<int>(std::lround(bbox->y2_))));
    iou_bbox->score = bbox->score_;
    iou_bbox->box_id = i;
...
void IOU2::copy_inrects_to_out(BaseDataVector *rects_msg,
  ...
  for (size_t i = 0; i < size; i++) {
    auto &in_rect = rects_msg->datas_[i];
    // HOBOT_CHECK("BBox" == in_rect->type_);
    auto inbox = std::static_pointer_cast<BBox>(in_rect);

    std::shared_ptr<BBox> outbox(new BBox());
    outbox->type_ = "BBox";
    outbox->id_ = -1;
    outbox->x1_ = std::max(0,
                         static_cast<int>(std::lround(inbox->x1_)));
    outbox->y1_ = std::max(0,
                         static_cast<int>(std::lround(inbox->y1_)));
    outbox->x2_ = std::min(img_width - 1,
                         static_cast<int>(std::lround(inbox->x2_)));
    outbox->y2_ = std::min(img_height - 1,
                         static_cast<int>(std::lround(inbox->y2_)));
    outbox->score_ = inbox->score_;
...
void IOU2::track_to_rects(const time_t &time_stamp,
  ...
  for (const auto &tracklet : tracklet_list) {
    if (tracklet->state == hobot::iou_mot::TrackLet::Deleted) {
      ...
      track_id->value_ = tracklet->track_id;
      ...
      if (target_out) {
        ...
        if (flag && box_id >= 0 &&
          ...
          auto bbox =
              std::static_pointer_cast<BBox>((*p_out_rects)[box_id]);
          bbox->x1_ = target_out->body_bbox->x1;
          bbox->x2_ = target_out->body_bbox->x2;
          bbox->y1_ = target_out->body_bbox->y1;
          bbox->y2_ = target_out->body_bbox->y2;
          bbox->id_ = tracklet->track_id;

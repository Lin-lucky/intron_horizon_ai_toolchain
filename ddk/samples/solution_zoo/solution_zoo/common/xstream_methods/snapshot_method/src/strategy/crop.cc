...
std::vector<BaseDataPtr> Crop::CropAndPost(const std::vector<BaseDataPtr> &in) {
  ...
  auto pframe = std::static_pointer_cast<xstream::ImageFrame>(img_frame);
  auto snapshot_info_list = std::make_shared<BaseDataVector>();
  out.push_back(snapshot_info_list);
  for (size_t i = 0; i < item_size; i++) {
    auto pbbox = std::static_pointer_cast<xstream::BBox>(box_list->datas_[i]);
    if (pbbox->state_ == DataState::VALID) {
      ...
      snaps_info.push_back(SnapShotInfo::GetSnapShotInfo(
          pframe, 0, pbbox, snapshot_config_param_.get(),
          onetrack_user_datas));

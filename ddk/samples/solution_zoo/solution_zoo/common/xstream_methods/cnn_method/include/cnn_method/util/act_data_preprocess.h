 ...
 public:
  ...
  void GetClipFeatByEnd(std::shared_ptr<BaseDataVector> kpses,
    ...
    for (auto& idx : clip_idxs) {
      auto tmp_kps_value =
          std::static_pointer_cast<Landmarks>(feats_->datas_[idx])->values_;
      auto p_tmp_kps = std::make_shared<Landmarks>();
...
class ActDataPreprocess
    : public horizon::vision::CSingleton<ActDataPreprocess> {
 public:
  // ActDataPreprocess();
  ...
  void Update(std::shared_ptr<BaseData> box,
    ...
    auto p_box = std::static_pointer_cast<BBox>(box);
    auto track_id = p_box->id_;
    if (track_buffers_.find(track_id) == track_buffers_.end()) {
      FeatureSequenceBuffer buffer(buff_len_);
      track_buffers_[track_id] = buffer;
    }
    auto kps_value = std::static_pointer_cast<Landmarks>(kps)->values_;
    auto p_kps = std::make_shared<Landmarks>();
    p_kps->values_ = kps_value;
  ...
  void NormKps(std::shared_ptr<BaseDataVector> kpses,
    ...
    auto first_kps =
        std::static_pointer_cast<Landmarks>(kpses->datas_[0]);
    Point center;
    if (type == xstream::LmkSeqOutputType::FALL) {
      Point left_hip = first_kps->values_[11];
      Point right_hip = first_kps->values_[12];
      float hip_x = left_hip.x_ * 0.5 + right_hip.x_ * 0.5;
      float hip_y = left_hip.y_ * 0.5 + right_hip.y_ * 0.5;
      center.x_ = hip_x;
      center.y_ = hip_y;
    } else if (type == xstream::LmkSeqOutputType::GESTURE) {
      center = first_kps->values_[9];
    }
    LOGD << "center_x: " << center.x_ << ", center_y: " << center.y_;
    max_score_ = -1;
    for (auto p_kps : kpses->datas_) {
      auto kps = std::static_pointer_cast<Landmarks>(p_kps);
    ...
    for (auto p_box : boxes->datas_) {
      auto box = std::static_pointer_cast<BBox>(p_box);
    ...
    for (size_t i = 0; i < disappeared_track_ids->datas_.size(); ++i) {
      auto disappeared_track_id =
          std::static_pointer_cast<xstream::Attribute_<uint32_t>>(
                  disappeared_track_ids->datas_[i])->value_;

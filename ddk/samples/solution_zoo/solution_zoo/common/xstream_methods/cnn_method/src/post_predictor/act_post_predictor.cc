...
std::vector<BaseDataPtr> ActPostPredictor::DefaultVaule(int size) {
  ...
  auto def_val = std::make_shared<xstream::Attribute_<int>>();
  def_val->value_ = -1;
  def_val->score_ = 0;
...
std::vector<BaseDataPtr> ActPostPredictor::TargetPro(
  ...
  for (size_t i = 0; i < mxnet_outs.size(); ++i) {
    ...
    if (output_type_ == LmkSeqOutputType::FALL) {
      ...
      act_ret->score_ = act_rets[target_group_];
      ...
      if (max_group_score >= threshold) {
        act_ret->value_ = max_group_index;
        act_ret->score_ = max_group_score;
      } else {
        act_ret->value_ = 0;
        act_ret->score_ = max_group_score;
      }
    ...
    LOGD << "idx: " << i << " act_value: " << act_ret->value_
         << " act_score: " << act_ret->score_;
    if (en_dump_feature_) {
      std::fstream pred_ret_file;
      pred_ret_file.open("pred_ret.txt", std::ios_base::app);
      pred_ret_file << "[" << act_ret->value_ << ", "
                    << act_ret->score_ << "]";
      if (dim_idx != dim_size - 1) {
        pred_ret_file << ", ";
      }

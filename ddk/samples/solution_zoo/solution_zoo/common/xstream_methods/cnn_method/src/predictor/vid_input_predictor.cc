...
typedef xstream::Attribute_<std::string> XRocString;
...
void VidInputPredictor::Do(CNNMethodRunData *run_data) {
  ...
  {  // one frame
    ...
    auto input_ptr =
        std::static_pointer_cast<xstream::DataArray_<float>>(first_item);
    for (size_t i = 0; i < input_ptr->values_.size(); ++i) {
      frame_input.emplace_back(input_ptr->values_[i]);
    }
...

    {
      ...
      for (int j = 0; j < layer_size; j++) {
        ...
        xroc_msg->value_ = socket_str[ret_index];
        std::cout << "result  = " << xroc_msg->value_ << std::endl;

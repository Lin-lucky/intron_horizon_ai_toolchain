...
void ImgInputPredictor::Do(CNNMethodRunData *run_data) {
  ...
  {  // one frame
    ...
    uint32_t w = pyramid->Width(pyramid_layer);
    uint32_t h = pyramid->Height(pyramid_layer);
    ...
    for (uint32_t roi_idx = 0; roi_idx < handle_num; roi_idx++) {
      std::shared_ptr<BBox> p_roi;
      p_roi = std::static_pointer_cast<BBox>(rois->datas_[roi_idx]);
      ...
      if (NormalizeRoi(p_roi.get(),
                        norm_box,
                        norm_params_,
                        w,
                        h,
                        filter_method_)) {
        LOGD << "norm roi error, box: [" << p_roi->x1_ << ", " << p_roi->y1_
             << ", " << p_roi->x2_ << ", " << p_roi->y2_ << "]";
        continue;
      }
      ...
      int tmp_src_w = 0, tmp_src_h = 0, tmp_dst_w = 0, tmp_dst_h = 0;
      {
        ...
        // crop
        tmp_src_data =
            reinterpret_cast<uint8_t *>(pyramid->Data(pyramid_layer));
        tmp_src_uv_data =
            reinterpret_cast<uint8_t *>(pyramid->DataUV(pyramid_layer));
        tmp_src_size =
            pyramid->Stride(pyramid_layer) * pyramid->Height(pyramid_layer);
        tmp_src_uv_size = pyramid->StrideUV(pyramid_layer) *
                          pyramid->Height(pyramid_layer) / 2;
        tmp_src_w = w;
        tmp_src_h = h;
        src_1_stride = pyramid->Stride(pyramid_layer);
        src_2_stride = pyramid->StrideUV(pyramid_layer);
        ...
        int ret = HobotXStreamCropYUVImage(
            ...
            src_2_stride, IMAGE_TOOLS_RAW_YUV_NV12, norm_box->x1_,
            norm_box->y1_, norm_box->x2_ - 1, norm_box->y2_ - 1, &tmp_dst_data,
            &tmp_dst_size, &tmp_dst_w, &tmp_dst_h, &dst_1_stride,
            &dst_2_stride);
        HOBOT_CHECK(ret == 0)
            << "crop img failed"

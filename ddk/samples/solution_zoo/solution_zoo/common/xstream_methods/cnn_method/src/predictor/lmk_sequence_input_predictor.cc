...
void LmkSeqInputPredictor::Do(CNNMethodRunData *run_data) {
  ...
  {  // one frame
    ...
    for (int roi_idx = 0; roi_idx < box_num; ++roi_idx) {
      auto p_roi =
        std::static_pointer_cast<BBox>(rois->datas_[roi_idx]);
      auto p_kps =
        std::static_pointer_cast<Landmarks>(
            kpses->datas_[roi_idx]);
      if (p_roi->state_ != xstream::DataState::VALID ||
          p_kps->values_.size() != static_cast<uint32_t>(kps_len_) ||
          roi_idx >= handle_num) {
        continue;
      } else {
        // pre-process
        int8_t* feature_buf_ = nullptr;
        {
          ...
          data_processor_.Instance().GetClipKps(cached_kpses, p_roi->id_,
              ...
              for (int hh = 0; hh < input_h; ++hh) {
                for (int ww = 0; ww < input_w; ++ww) {
                  ...
                  tensor.Set(nn, hh, ww, 2,
                             cur_kps->values_[kps_idx].score_);
                }
              }
            }
            if (enable_dump) {

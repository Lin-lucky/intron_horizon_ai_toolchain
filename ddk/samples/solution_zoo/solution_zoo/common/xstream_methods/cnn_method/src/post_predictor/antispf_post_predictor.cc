...
BaseDataPtr AntiSpfPostPredictor::FaceAntiSpfPostPro(
  ...
  auto anti_spf = std::make_shared<xstream::Attribute_<int>>();
  if (mxnet_outs.size() == 0 || mxnet_outs[0].size() == 0) {
    anti_spf->value_ = -1;
    anti_spf->score_ = 1.0f;
    anti_spf->state_ = DataState::INVALID;
  } else {
    auto mxnet_out = reinterpret_cast<const float *>(mxnet_outs[0].data());
    if (channel_size == 1) {
      anti_spf->score_ = SigMoid(mxnet_out[0]);
      anti_spf->value_ = anti_spf->score_ > anti_spf_threshold_ ? 1 : 0;
    } else {
      std::vector<float> attrs = {mxnet_out[0], mxnet_out[1]};
      SoftMax(attrs);
      anti_spf->score_ = attrs[1];
      anti_spf->value_ = anti_spf->score_ > anti_spf_threshold_ ? 1 : 0;
    }
  }

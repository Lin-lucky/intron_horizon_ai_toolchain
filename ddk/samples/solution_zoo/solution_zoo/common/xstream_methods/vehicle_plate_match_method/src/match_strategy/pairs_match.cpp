...
std::vector<int> PairsMatch::GetRelativePos(
    const xstream::BBox &large_bbox, const xstream::BBox &small_bbox) const {
  float large_cx = BBoxCenterX(large_bbox);
  float large_cy = BBoxCenterY(large_bbox);
  float large_w = large_bbox.x2_ - large_bbox.x1_;
  float large_h = large_bbox.y2_ - large_bbox.y1_;

  float small_cx = std::max(0.f, BBoxCenterX(small_bbox) - large_bbox.x1_);
  float small_cy = std::max(0.f, BBoxCenterY(small_bbox) - large_bbox.y1_);
...
float PairsMatch::ComputePairDistance(const xstream::BBox &large_bbox,
...
void PairsMatch::UpdateStatMat(const xstream::BBox &vehicle_bbox,
...
void PairsMatch::PrepareMatchIDs(const VehicleListPtr &vehicle_list,
  ...
  for (std::size_t i = 0; i < vehicle_list->size(); ++i) {
    xstream::BBox vehicle_bbox((*(*vehicle_list)[i].bbox));
    for (std::size_t j = 0; j < plate_list->size(); ++j) {
      xstream::BBox plate_bbox((*(*plate_list)[j].bbox));
...
void PairsMatch::ProcessOneToOneMatch(VehicleListPtr &vehicle_list,
  ...
  for (size_t i = 0; i < vehicle_list->size(); ++i) {
    ...
    if (vehicle_match_ids_[i].size() == 1 &&
      ...
      xstream::BBox plate_bbox(*plate.bbox);
...
void PairsMatch::ProcessMultiToMultiMatch(VehicleListPtr &vehicle_list,
  ...
  for (const auto &itr1 : vehicle_match_ids_) {
    xstream::BBox vehicle_bbox((*(*vehicle_list)[itr1.first].bbox));
    for (const auto &itr2 : itr1.second) {
      xstream::BBox plate_bbox((*(*plate_list)[itr2].bbox));

...
TEST(VehicleMatchModule, PairMatch) {
  ...
  while (std::getline(input_fs, line)) {
    ...
    for (int i = 0; i < static_cast<int>(vehicle_data.size()); ++i) {
      ...
      BBoxPtr bbox =
          std::make_shared<xstream::BBox>(tmp[0], tmp[1], tmp[2], tmp[3]);
      Vehicle veh_tmp;
      veh_tmp.bbox = bbox;
      veh_tmp.bbox->id_ = vehicle_data[i]["track_id"].asUInt64();
    ...
    for (const auto &itr : *vehicle_list) {
      ...
      veh["bbox"].append(static_cast<int>(itr.bbox->x1_));
      veh["bbox"].append(static_cast<int>(itr.bbox->y1_));
      veh["bbox"].append(static_cast<int>(itr.bbox->x2_));
      veh["bbox"].append(static_cast<int>(itr.bbox->y2_));
      veh["track_id"] = itr.bbox->id_;

      if (itr.plate != nullptr) {
        veh["plate_box"] = Json::Value();
        veh["plate_box"]["bbox"] = Json::Value(Json::arrayValue);

        veh["plate_box"]["bbox"].append(static_cast<int>(itr.plate->bbox->x1_));
        veh["plate_box"]["bbox"].append(static_cast<int>(itr.plate->bbox->y1_));
        veh["plate_box"]["bbox"].append(static_cast<int>(itr.plate->bbox->x2_));
        veh["plate_box"]["bbox"].append(static_cast<int>(itr.plate->bbox->y2_));

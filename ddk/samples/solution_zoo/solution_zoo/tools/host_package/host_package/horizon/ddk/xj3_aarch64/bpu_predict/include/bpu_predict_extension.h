...
// define loginterface for user log
typedef void (*LogInterface)(const char *);
...
typedef struct hb_BPU_MODEL_NODE_S {
  ...
  float *scales;  // only used for output node
  int scale_len;
} BPU_MODEL_NODE_S;
...
int HB_BPU_registerLogInterface(LogInterface error_log_interface,
                                LogInterface warning_log_interface,
                                LogInterface info_log_interface,
                                LogInterface debug_log_interface);
